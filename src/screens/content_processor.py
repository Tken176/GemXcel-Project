        ...
        if os.path.exists(api_file):
            try:
                with open(api_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    api_candidate = data.get("API")
                    if isinstance(api_candidate, str) and api_candidate.startswith("AI") and len(api_candidate) > 20:
                        self.api_key = api_candidate.strip()
            except Exception as e:
                print(f"Lỗi đọc API_AI.json: {e}")

        if not self.api_key:
            raise ValueError("API key không hợp lệ hoặc không tìm thấy trong API_AI.json!")
...
1. Tạo bài học theo thứ tự hợp lý (từ cơ bản đến nâng cao, từ mở bài đến kết thúc) 
2. Tiêu đề ngắn gọn, phản ánh nội dung chính
3. Nội dung tóm tắt 230-350 từ, tập trung vào ý quan trọng
4. {self.questions_per_lesson} câu hỏi với độ khó phân bố đều:
...
"""
    ...
    def generate_quiz(self, lessons: List[Dict], total_questions: int = 10) -> Dict:
        ...
        # Đảm bảo mỗi độ khó có ít nhất 10 câu hỏi
