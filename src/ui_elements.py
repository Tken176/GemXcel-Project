...
class TextButton:
    def __init__(self, x, y, text, callback,colors=None,font=None, click_sound=None):
        ...
        colors    : dict {"normal": (R,G,B), "hover": (R,G,B)}
        callback  : hàm gọi khi click
        click_sound: âm thanh click (pygame.mixer.Sound)
        """
        self.x = x
        self.y = y
        self.text = text
    ...
    def draw(self, surface):
        mouse_pos = pygame.mouse.get_pos()
        if self.rect.collidepoint(mouse_pos):
            self.text_surface = self.font.render(str(self.text), True, (90, 160, 90))
            self.is_hover = True
        else:
            self.text_surface = self.font.render(str(self.text), True, (20, 60, 20))
            self.is_hover = False
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and self.rect.collidepoint(event.pos):
            current_time = time.time()
            if current_time - _last_click_time > 0.3:  # chống double click
                if self.click_sound:
                    self.click_sound.play()
                self.callback()
                _last_click_time = current_time
...
# biến toàn cục để chống spam click
_last_click_time = 0
def draw_button(surface, text, x, y, width, height, color, hover_color, action=None):
    # This function is now redundant if you use the Button class,
    # but I'll keep it as a wrapper if some parts of the code still rely on it directly.
